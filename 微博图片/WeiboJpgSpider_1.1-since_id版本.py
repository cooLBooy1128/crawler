...
def get_url(base_url,page_json,url,page_api_urls,p_count):
    if page_json:
        if page_json.get('data').get('cards'):
            since_id = page_json.get('data').get('cardlistInfo').get('since_id')
            if since_id:
                since_id=str(since_id)
                print(re.match('(.*?&since_id=).*',base_url).group(1) + since_id)
                p_count+=1
                page_api_urls.append(re.match('(.*?&since_id=).*',base_url).group(1) + since_id)
                return re.match('(.*?&since_id=).*',base_url).group(1) + since_id,p_count
            else:
                return None,p_count
        else:
            since_id=str(int(re.search('since_id=(.*)',url).group(1))-500000000000)
            print(re.match('(.*?&since_id=).*',base_url).group(1) + since_id)
            return re.match('(.*?&since_id=).*',base_url).group(1) + since_id,p_count
    else:
        return None,p_count
        
def set_base_url(uid,since_id):
    return 'https://m.weibo.cn/api/container/getIndex?type=uid&value={}&containerid=107603{}&since_id={}'.format(uid,uid,since_id)

def get_user_name(page_json):
    if page_json:
        return page_json.get('data').get('cards')[0].get('mblog').get('user').get('screen_name')

def main(uid,since_id,page_count_max=700,jpg_count=None):
    '''
    用户id；
    ...
    要存储的图片数量，默认存储所有图片。
    '''
    base_url=set_base_url(uid,since_id)
...
if __name__ == '__main__':
    ...
    要存储的图片数量，默认存储所有图片。
    '''
    main(5649085126,since_id=None,page_count_max=700,jpg_count=0)
