...
def get_items(page_json):
    if page_json.get('data'):
        for item in page_json.get('data'):
            if item.get('image_list') and item.get('image_count') > 0:
                yield{
...
def main(offset):
    ...
    global count
    count=1
    page_json=get_page_json(offset[0],offset[1])
    for item in get_items(page_json):
        title=item.get('title')
        in_keyword=item.get('keyword')
        datetime=item.get('datetime')
...
if __name__ == '__main__':
    ...
    #不使用map方法，通过for循环apply_async方法添加多个子进程。
    '''offsets=[i*20 for i in range(START_PAGE-1,END_PAGE)]
    for offset in offsets:
        pool.apply_async(main,(KEYWORD,offset))'''

    offsets=[(KEYWORD,i*20) for i in range(START_PAGE-1,END_PAGE)]
